module "site-on-openshift" {
  count                   = var.site_on_openshift_count
  type                    = "openshift"
  source                  = "./site"
  f5xc_cluster_name       = format("%s-site-on-openshift-%d", var.project_prefix, count.index)
